...
## 25. ✅ step8 fetch-skid fix の 50MHz timing 回復 = dup_ahead 比較の narrow 化 (第20セッション)

step8 (FTQ block fetch) の reload-settle 由来 4 バイトフェッチ skid crash の根治 fix(A) (memory
[[step8-fetch-skid-rootcause]]) が、薄い 50MHz default margin (step11 = +0.355ns MET) を **P&R 撹乱で
-0.084ns NOT met (48 failing)** に悪化させていた。fix 自体は critical path 外 (worst は load->branch->redirect
の既存 binding) で、追加した `imem_addr != bfpc` の **64bit 全比較**が placement を乱しただけ。

- **修正** (`src/rtl/core/rv_core.sv`): `wire dup_ahead = hb_dup & (imem_addr[5:2] != bfpc[5:2]);` に narrow 化。
  reload-settle over-serve は常に `imem_addr = bfpc + 4` (bit2 が違う) → `[5:2]` 不一致 → 抑制 (正)。IF-PTW hold は
  `imem_addr == bfpc` → `[5:2]` 一致 → 抑制せず (正、livelock 回避)。dup (re-serve, `bfpc == hb_last_pc`) 文脈では
  64B 超ストライド不可ゆえエイリアス無し。**機能完全等価**(全機能ゲート再 PASS: compliance RV64 117/RV32 88、
  cache_soc 6/6 (skid 直り)、RANDLAT crash-free FCHK=0、Linux userspace chars=11428)。
- **実 impl_1 routed (50MHz, BD fresh 再生成)**: **WNS +0.382ns, 0 failing, clk_fpga_0 20ns=50MHz, all met**
  (step11 baseline +0.355 を上回り回復)。worst = `gen_dcache.u_dc/data_reg` (D$ BRAM) ->
  `redir_pend_tgt_q_reg[58]` (load->branch->redirect, CARRY4=10; 既存 binding)。fix セル (dup_ahead/ftq_pop/
  hb_last) は report に皆無 = critical path 外を確定。
- **全周波数 50MHz 整合**: `set_pl_freq.py --refresh-ps7` で ps7_init を 50MHz XSA から更新 (0x00500500 DIV25=40MHz
  -> **0x00400500 DIV20=50MHz**)、`make fw-opensbi-hw fw-linux-hw` で firmware を 50MHz DT 埋込み再ビルド。
- **教訓**: 薄マージン flow では「critical path 外の追加ロジックでも placement 撹乱で WNS が動く」。機能 fix を
  入れる際は **比較幅・ロジック量を最小化** (ここでは 64bit -> 4bit) して撹乱を抑える。closure 判定は §24.5 同様
  実 impl_1 routed WNS で。
