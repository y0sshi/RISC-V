...
#(
    ...
    always @(*) begin
        ...
        if (ds_nan) begin
            ...
            // Inline LZC for 23-bit fraction (loop-based priority encoder:
            // scan LSB->MSB, last/highest set bit wins; all-zero -> 23).
            ds_lz = 5'd23;
            for (int i = 0; i <= 22; i++)
                if (ds_f[i]) ds_lz = 5'(22 - i);   // ds_f[22]->0 ... ds_f[0]->22
