...
module rv_fpu_add (
    ...
    always_comb begin
        ...
        if (sum[27]) begin
            ...
            norm_sig    = {1'b0, sum[27:4], sum[3], sum[2], sum[1] | sum[0]};
            norm_exp    = el + 9'd1;
        end else if (sum[26:0] == 27'h0) begin
            // Exact zero from subtraction
            is_exact_zero = 1'b1;
        end else if (!sum[26]) begin
            // Need to shift left (subtraction cancellation)
            // Max shift: el - 1 (to keep exp >= 1), or further into subnormal
            if ({4'b0, lzc} >= el) begin
                // Result is subnormal: shift until exp reaches 0
                shift_amt = (el > 0) ? el[4:0] - 5'd1 : 5'd0;
                norm_exp  = 9'd0;
            end else begin
                shift_amt = lzc;
                norm_exp  = el - {4'b0, lzc};
            end
            norm_sig = sum[26:0] << shift_amt;
        end
        // else: sum[26]=1 and no overflow — already normalized, norm_sig=sum[26:0]
    end
    ...
    always_comb begin
        case (rm)
            ...
            3'b010: round_up = (G | R | S_bit) & sum_sign;      // RDN
            3'b011: round_up = (G | R | S_bit) & !sum_sign;     // RUP
    ...
    always_comb begin
        ...
        if (a_nan || b_nan) begin
            ...
            fflags[4] = a_snan | b_snan;  // NV if any sNaN
            ...
            if (final_exp >= 9'd255) begin
                ...
                fflags[0] = 1'b1;  // NX
                case (rm)
                    3'b001: result = {sum_sign, 8'hFE, 23'h7FFFFF};  // RTZ -> max finite
                    3'b010: result = sum_sign ? {1'b1,8'hFF,23'h0} : {1'b0,8'hFE,23'h7FFFFF};
                    3'b011: result = sum_sign ? {1'b1,8'hFE,23'h7FFFFF} : {1'b0,8'hFF,23'h0};
                    default: result = {sum_sign, 8'hFF, 23'h0};  // round to infinity
                endcase
            end else if (final_exp == 0 && norm_exp == 0) begin
                // Subnormal result
                result    = {sum_sign, 8'h00, final_frac};
                fflags[1] = (G | R | S_bit);  // UF
                fflags[0] = (G | R | S_bit);  // NX
            end else begin
                result = {sum_sign, final_exp[7:0], final_frac};
